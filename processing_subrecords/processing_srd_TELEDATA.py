...
def pr_EGTS_TELEDATA_SERVICE(srds, data_for_db):
    for srd in srds:
        ...
        match hex_to_dec(dict_srd["SRT"]):
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_RECORD_RESPONSE.value:
                dict_srd["SRD"], data_srd["CRN"] = param_byte(dict_srd["SRD"], 2, True)
                dict_srd["SRD"], data_srd["RST"] = param_byte(dict_srd["SRD"], 1, False)

            # Это надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_POS_DATA.value:
            ...
            # это надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_EXT_POS_DATA.value:
                dict_srd["SRD"], tmp_byte = param_byte(dict_srd["SRD"], 1, False)
            ...
            # это надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_AD_SENSORS_DATA.value:
                dict_srd["SRD"], tmp_byte = param_byte(dict_srd["SRD"], 1, False)
                ...
                for ind_bit in range(len(bit_list_DIOE)):
                    if int(bit_list_DIOE[ind_bit]) == 1:
                        dict_srd["SRD"], data_srd[f"ADIO{ind_bit+1}"] = param_byte(dict_srd["SRD"], 1, False)

                bit_list_ASFE = bit_list_ASFE[::-1]
                for ind_bit in range(len(bit_list_ASFE)):
                    if int(bit_list_ASFE[ind_bit]) == 1:
                        dict_srd["SRD"], data_srd[f"ANS{ind_bit + 1}"] = param_byte(dict_srd["SRD"], 3, False)

            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_COUNTERS_DATA.value:
                # dict_srd["SRD"], tmp_byte = param_byte(dict_srd["SRD"], 1, False)
            ...
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_ACCEL_DATA.value:
                ...
                while len(dict_srd["SRD"]) > 0:
                    i += 1
                    srd_dict_data = {}
                    dict_srd["SRD"], data_srd[f"ADS{i}"] = param_byte(dict_srd["SRD"], 8, False)

                    data_srd[f"ADS{i}"], srd_dict_data["RTM"] = param_byte(data_srd[f"ADS{i}"], 2, True)
                    data_srd[f"ADS{i}"], srd_dict_data["XAAV"] = param_byte(data_srd[f"ADS{i}"], 2, True)
                    data_srd[f"ADS{i}"], srd_dict_data["YAAV"] = param_byte(data_srd[f"ADS{i}"], 2, True)
                    data_srd[f"ADS{i}"], srd_dict_data["YAAV"] = param_byte(data_srd[f"ADS{i}"], 2, True)

                    data_srd[f"ADS{i}"] = srd_dict_data

            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_STATE_DATA.value:
                pass
            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_LOOPIN_DATA.value:
                pass
            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_ABS_DIG_SENS_DATA.value:
                pass
            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_ABS_AN_SENS_DATA.value:
                pass

            # это надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_ABS_CNTR_DATA.value:
                dict_srd["SRD"], data_srd["CN"] = param_byte(dict_srd["SRD"], 1, False)
                dict_srd["SRD"], data_srd["CNV"] = param_byte(dict_srd["SRD"], 3, False)

            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_ABS_LOOPIN_DATA.value:
                pass

            # это надо.
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_LIQUID_LEVEL_SENSOR.value:
                dict_srd["SRD"], tmp_byte = param_byte(dict_srd["SRD"], 1, False)
            ...
            # это не надо
            case Tsr_EGTS_TELEDATA_SERVICE.EGTS_SR_PASSENGERS_COUNTERS.value:
                pass
