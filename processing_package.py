...
def processing_EGTS_PT_APPDATA(byte_string, data_for_db, dict_data_sfrd=None):
    ...
    while len(byte_string) > 0:
        try:
            ...
            i += 1
            dict_data_sfrd[f"RID={i}"] = dict_data
...
def processing_subrecord(rids, data_for_db):
    for rid in rids.keys():
        ...
        match hex_to_dec(dict_rid["RST"]):
            ...
            case Types_services.EGTS_COMMANDS_SERVICE.value:
                print("EGTS_COMMANDS_SERVICE")

            case Types_services.EGTS_FIRMWARE_SERVICE.value:
                print("EGTS_FIRMWARE_SERVICE")

            case Types_services.EGTS_ECALL_SERVICE.value:
